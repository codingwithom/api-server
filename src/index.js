...
async function getPwToken(forceRefresh = false) {
  if (!forceRefresh && pwTokenMemoryCache.token && pwTokenMemoryCache.expiresAt > Date.now()) {
  ...
  if (!forceRefresh && pwTokenMemoryCache.token) return pwTokenMemoryCache.token;
  return FALLBACK_PW_TOKEN || BACKUP_PW_TOKEN;
}
...
async function fetchChapterContents(batchId, subjectId, chapterId, token, allowFallback = true, chapterTitle = "") {
  ...
  if (cached && cached.expiresAt > Date.now() && cached.data && (cached.data.totalLectures > 0 || (cached.data.lectures && cached.data.lectures.length > 0))) {
    return cached.data;
  }
  ...
  try {
    ...
    if (allowFallback && rawVideos.length === 0 && rawNotes.length === 0 && rawDpps.length === 0) {
      ...
      try {
        const candidateToken = (BACKUP_PW_TOKEN && BACKUP_PW_TOKEN !== token) ? BACKUP_PW_TOKEN : await getPwToken(true);
        if (candidateToken && candidateToken !== token) {
          const retryResult = await fetchChapterContents(batchId, subjectId, chapterId, candidateToken, false, chapterTitle);
...
async function fetchPwSchedule(batchId, date, month, startDate, endDate) {
  ...
  if (!fullSchedule || fullSchedule.expiresAt <= Date.now()) {
    ...
    if (token) {
      try {
        ...
        for (let i = 0; i < pages.length; i += 4) {
          ...
          if (!chunkHasData) break;
        }

        // Also fetch exact date directly to guarantee today's schedule is immediately available if missing
        if (date && /^\d{4}-\d{2}-\d{2}$/.test(date) && !rawItems.some(it => (it.date || it.startTime || "").startsWith(date))) {
          try {
    ...
    // 1b. If token was stale, try backup token or force refreshed token
    if (rawItems.length === 0) {
      try {
        const altToken = (BACKUP_PW_TOKEN && BACKUP_PW_TOKEN !== token) ? BACKUP_PW_TOKEN : await getPwToken(true).catch(() => "");
    ...
    const hasRealItems = list.some(item => item && item.id && !String(item.id).startsWith("synth-"));
    fullSchedule = {
      list,
      hasRealItems,
      expiresAt: Date.now() + (hasRealItems ? PW_SCHEDULE_TTL : 5000)
    };
  ...
  async fetch(request, env, ctx) {
    ...
    if (pathname === "/api/verify-captcha") {
      ...
      try {
        ...
        const secret = env?.RECAPTCHA_SECRET_KEY || atob("NkxlYjE4NHRBQUFBQUlBTThZazZ0dDBnWl9LckcwY0NuN3U3ckNaVA==");
        if (!secret) {
    ...
    if (pathname === "/api/pw-debug-test") {
      try {
        ...
        // Variant C: Direct without Authorization header (just to see if status changes)
        ...
        return jsonResponse({
          ...
          variantA_XForwardedFor: resA,
          variantB_Cookie: resB,
          variantC_NoAuth: resC
        });
      } catch (err) {
    ...
    if (pathname === "/api/pw-schedule" || pathname === "/api/pw-batch-schedule") {
      const batchId = url.searchParams.get("batchId") || "";
      ...
      try {
        ...
        const hasRealSchedule = Boolean(data && (data.hasRealItems || (Array.isArray(data.schedules) && data.schedules.some(s => s && s.id && !String(s.id).startsWith("synth-")))));
        return jsonResponse(data, 200, { "Cache-Control": hasRealSchedule ? "public, max-age=600" : "no-cache, no-store, must-revalidate" });
      } catch (err) {
